...
#define LOWEST_PRECOMPUTED_SCORE 2
...
// This player has a more sophistocated strategy. It uses a precomputed lookup table to decide on the best target, then considers whether it would be better to intentionally bust.
class PlayerAdvanced :
	public Player
{
public:
	PlayerAdvanced(std::string _name, int _skillLevel, Scoreboard* _scoreBoard);
	~PlayerAdvanced();
